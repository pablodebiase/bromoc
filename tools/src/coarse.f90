!    COARSE - Converts atomistic DNA structure into Coarse Grained DNA
...
implicit none
integer i,j,k,tnf
character crdfile*256,dcdfile*256,mskfile*256,line*1024,frmt*4
integer ul(256),ll(256),num,narg,arg
integer*1 stf,cf,wof
character bs*8
...
elseif (cf.eq.3) then
  write(*,'(A)') 'dcd ...'
  call readdcdhead(dcdfile,tnf,2)
  call readdcdbody(2)
elseif (cf.eq.4) then
  write(*,'(A)') 'dcde ...'
  call readdcdhead(dcdfile,tnf,2)
...
subroutine readdcdhead(dcdfile,tnf,u)
use comun
implicit none
integer icntrl(20),itemp,kode,tnf,u
...
prname='COARSE'
prver='version 1.1'
prdesc='Converts atomistic DNA structure into Coarse Grained DNA'
author='Pablo M. De Biase (pablodebiase@gmail.com)'
startdate='01 Apr 2011'
lastdate='27 Jan 2014'
